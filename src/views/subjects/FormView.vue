<template>
  <Form @submit="onSubmit">
    <div class="mb-3">
      ...
      <Field class="form-control" v-model="description" name="description" type="text" placeholder="Digite a descrição"
        :rules="validate" />
      <ErrorMessage name="description" class="errorMessage" />
    </div>
    <div class="mb-3 text-end">
      <router-link class="btn btn-secondary me-2" to="/assuntos">Voltar</router-link>
...
onMounted(async () => {
  if (route.params.id) {
    ...
    description.value = store.getSubject.description;
  }
})
</script>